...
def get_app_version(app_type):
    ...
    # 避免并发造成多余查询
    with cache_lock:
        version = db.slave.system_version.filter(type=app_type).order_by('release_date', 'id').select('must','api').last()
...
def add_access_log(request):
    ...
    platfrom = request.META.get('HTTP_PLATFORM', '')
    ...
    logging.info("path=%s, ip=%s,agent=%s,user_id=%s, platfrom=%s" %
                  (path,ip,agent,user_id, platfrom))
    #TODO: 写redis
