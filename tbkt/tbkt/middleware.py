...
from libs.utils import ajax, casts, loads, auth, thread_pool
...
def get_userid(request):
    ...
    if not hasattr(request, '_userid'):
        token = request.QUERY.get("tbkt_token") or request.META.get('HTTP_TBKT_TOKEN') or request.COOKIES.get('tbkt_token')
...
def get_user(request):
    """
    从cookie中获取user, 失败返回None
    """
    if not hasattr(request, '_user'):
        user_id = get_userid(request)
        request._user = com_user.get_user(user_id) if user_id else None
...
class AuthenticationMiddleware(object):
    ...
    def process_exception(self, request, exception):
        ...
        r = ajax.jsonp_fail(request, error="500", message="抱歉，服务器开小差了，请联系客服12556185")
        r.status_code = 500
        return r
